...
const DeliveryCommissionsReportFilter = ({
  ...
  return (
    <div className={`rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-opacity dark:border-white/10 dark:bg-slate-900 ${isPending ? "opacity-60" : ""}`}>
      <AdminSearchForm method="GET" className="space-y-3">
        <input type="hidden" name="tab" value="report" />
        {fromDate ? <input type="hidden" name="rFrom" value={fromDate} /> : null}
        {toDate ? <input type="hidden" name="rTo" value={toDate} /> : null}
        {customerId ? <input type="hidden" name="customerId" value={customerId} /> : null}
        {deliveryStaffId ? <input type="hidden" name="rStaffId" value={deliveryStaffId} /> : null}
        {unpaidOnly ? <input type="hidden" name="unpaidOnly" value="1" /> : null}
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <AdminSearchSubmitButton className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#1e3a5f] px-5 py-2 text-sm font-medium text-white hover:bg-[#162d4a] disabled:opacity-50 dark:bg-sky-700 dark:hover:bg-sky-600">
              แสดงรายงาน
