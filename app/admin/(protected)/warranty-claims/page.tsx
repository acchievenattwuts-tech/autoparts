...
const ClaimListPage = async ({
  ...
  const dateWhere: Prisma.WarrantyClaimWhereInput =
    fromParam || toParam
      ? {
          claimDate: {
  ...
  const filteredWhere: Prisma.WarrantyClaimWhereInput = {
    AND: [dateWhere, searchWhere, status ? { status: status as never } : {}],
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100">
          <p className="text-sm text-gray-500">
            แสดง <span className="font-medium text-gray-700">{claims.length} รายการ</span>
          </p>
        </div>
        {claims.length === 0 ? (
                ...
                {claims.map((claim, idx) => (
                  <tr
                    ...
                    className={`border-t border-gray-50 hover:bg-gray-50 transition-colors ${
                      claim.status === "CANCELLED" ? "opacity-50" : ""
                    }`}
                  >
                    <td className="py-2.5 px-4 text-gray-400 text-xs">{(pageNum - 1) * PAGE_SIZE + idx + 1}</td>
                    <td className="py-2.5 px-4">
                      <Link href={`/admin/warranty-claims/${claim.id}`} className="font-mono text-sm text-[#1e3a5f] hover:underline font-medium">
                        {claim.claimNo}
                      </Link>
                    </td>
                    <td className="py-2.5 px-4">
                    ...
                    <td className="py-2.5 px-4 text-gray-600 text-xs">{claim.supplierName ?? "—"}</td>
                    <td className="py-2.5 px-4 text-center">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLOR[claim.status]}`}>
                  ...
                  {formatDateThai(claim.claimDate)}
                    ...
                    <td className="py-2.5 px-4">
                      <div className="flex items-center gap-2 justify-end">
                        <PrintFromListButton href={`/admin/warranty-claims/${claim.id}/print`} />
                        <Link
                        ...
                        {canUpdate && claim.status !== "CANCELLED" && (
                          <>
                            <Link
                              href={`/admin/warranty-claims/${claim.id}`}
