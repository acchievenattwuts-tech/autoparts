...
import { z } from "zod";
import { diffEntity, getAuditActorFromSession, getRequestContext, safeWriteAuditLog,
} from "@/lib/audit-log";
...
import { assertPaymentsMatchTotal, clearDocumentPayments, derivePrimaryAccountId, parseDocumentPaymentRows, replaceDocumentPayments, toCashBankEntries, type DocumentPaymentRow,
...
async function resolvePaymentMethod(tx: TxClient, payments: DocumentPaymentRow[],
...
function parsePayments(formData: FormData, totalAmount: number) {
  try {
    ...
    return { error: error instanceof Error ? error.message : "รูปแบบช่องทางรับเงินไม่ถูกต้อง",
    } as const;
  }
}
...
async function activeRefundRefs(id: string): Promise<string[]> {
  const rows = await db.customerAdvanceRefund.findMany({
...
async function assertNoActiveReceiptRefs(tx: TxClient, id: string, action: "แก้ไข" | "ยกเลิก",
): Promise<void> {
...
async function assertNoActiveRefundRefs(
  tx: TxClient,
  id: string,
  action: "ยกเลิก",
): Promise<void> {
  const rows = await tx.customerAdvanceRefund.findMany({
    where: { customerAdvanceId: id, status: "ACTIVE" },
    select: { refundNo: true },
  });
  if (rows.length > 0)
    throw new Error(
      `ไม่สามารถ${action}ได้ เนื่องจากถูกอ้างอิงในเอกสารคืนเงินมัดจำ: ${rows.map((row) => row.refundNo).join(", ")}`,
    );
}
...
function parseForm(formData: FormData) {
  ...
  return parsed.success ? ({ data: parsed.data } as const)
...
export async function updateCustomerAdvance(id: string, formData: FormData,
  ...
  const refundRefs = await activeRefundRefs(id);
  ...
  try {
    ...
    await dbTx(async (tx) => {
      ...
      const lockedRefundCount = await tx.customerAdvanceRefund.count({
        where: { customerAdvanceId: id, status: "ACTIVE" },
      });
...
export async function cancelCustomerAdvance(formData: FormData,
): Promise<{ success?: boolean; error?: string }> {
  ...
  const refundRefs = await activeRefundRefs(existing.id);
  if (refundRefs.length)
    return {
      error: `ไม่สามารถยกเลิกได้ เนื่องจากถูกอ้างอิงในเอกสารคืนเงินมัดจำ: ${refundRefs.join(", ")}`,
    };
  try {
    const requestContext = await getRequestContext();
    ...
    await dbTx(async (tx) => {
      ...
      await assertNoActiveRefundRefs(tx, existing.id, "ยกเลิก");
