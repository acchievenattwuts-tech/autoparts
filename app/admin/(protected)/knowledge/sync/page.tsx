...
export default async function KnowledgeSyncPage() {
  ...
  const [jobs, chunks] = await Promise.all([
    ...
    db.$queryRaw<
      Array<{ approved: number; archived: number }>
    >`SELECT count(*) FILTER (WHERE status='APPROVED')::int AS approved, count(*) FILTER (WHERE status='ARCHIVED')::int AS archived FROM knowledge_documents`,
  ]);
  ...
  return (
    <div>
      ...
      <AdminSectionCard
        title="ประวัติงาน Sync"
        description="Product embedding-1 ไม่ได้อยู่ในตารางหรือกระบวนการนี้"
        bodyClassName="p-0"
      >
              ...
              {jobs.map((job) => (
                <tr key={job.id}>
                  <td className="px-4 py-3">
                    <Link
                      href={`/admin/knowledge/${job.revision.sourceId}`}
                      className="font-medium text-slate-900 hover:text-orange-600 dark:text-slate-100"
                    >
                      {job.revision.title}
                    </Link>
                    <p className="text-xs text-slate-500">
                      revision {job.revision.revisionNo}
                    </p>
                  </td>
                  <td className="px-4 py-3 text-slate-500">{job.trigger}</td>
                  <td className="px-4 py-3">
                    <span
                      className={
                    ...
                    {job.lastError && (
                      <p className="mt-1 max-w-md text-xs text-rose-500">
                        {job.lastError}
                      </p>
                    )}
                  </td>
                  <td className="px-4 py-3 text-slate-500">
                    {formatKnowledgeTimestamp(job.createdAt)} น.
                  </td>
                  <td className="px-4 py-3">
                    {job.status === "FAILED" ? (
                      <KnowledgeActions
