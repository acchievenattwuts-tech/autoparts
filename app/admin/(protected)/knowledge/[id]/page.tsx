...
export default async function KnowledgeDetailPage({
  ...
  return (
    <div className="space-y-5">
      <AdminPageHeader
        eyebrow={`${source.type} · revision ${revision.revisionNo}`}
        title={revision.title}
        description={`สถานะ: ${labels[revision.status] ?? revision.status}`}
        meta={
          revision.syncError ? (
            <span className="text-rose-600 dark:text-rose-300">
              Sync error: {revision.syncError}
            </span>
          ) : undefined
        }
        actions={
      ...
      {revision.rejectionReason && (
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-200">
          <strong>เหตุผลที่ไม่อนุมัติ:</strong> {revision.rejectionReason}
        </div>
      )}
      {editable ? (
        <KnowledgeEditor
        ...
        <>
          <AdminSectionCard
            title="ตัวอย่างเนื้อหาที่จะเผยแพร่"
            description="หน้านี้เป็นการอ่านอย่างเดียว สร้าง revision ใหม่ก่อนแก้ไข"
          >
            <div className="space-y-5">
              ...
              {content.highlights.length > 0 && (
                <ul className="rounded-xl bg-slate-50 p-4 text-sm leading-7 text-slate-700 dark:bg-white/5 dark:text-slate-300">
              ...
              {content.sections.map((section) => (
                <section key={section.heading}>
                  <div className="flex items-center gap-2">
                    <h3 className="font-kanit text-lg font-semibold text-slate-900 dark:text-slate-100">
                      {section.heading}
                    </h3>
                    {section.aiEnabled && revision.ragEnabled && (
                      <span className="rounded-full bg-sky-100 px-2 py-0.5 text-xs text-sky-700 dark:bg-sky-500/15 dark:text-sky-200">
                        AI ใช้ได้
                      </span>
                    )}
                  </div>
                  <div className="mt-2 space-y-2">
                    {section.body.map((line) => (
                      ...
                      >
                        {section.format === "BULLETS" ? `• ${line}` : line}
                      </p>
                    ))}
                  </div>
                </section>
          ...
          <AdminSectionCard title="ขอบเขต AI">
            <p className="text-sm leading-7 text-slate-700 dark:text-slate-300">
              {revision.answerScope}
            </p>
          </AdminSectionCard>
        </>
      )}
          ...
          {source.revisions.map((item) => (
            ...
            >
              ...
              <span className="text-xs text-slate-500">
                ...
                {formatKnowledgeTimestamp(item.updatedAt)} น. (เวลาไทย)
              </span>
            </div>
          ))}
        </div>
