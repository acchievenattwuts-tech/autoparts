...
function SnapshotUnavailable({ compact }: { compact: boolean }) {
  return (
    <section className={compact ? "space-y-3" : "space-y-4"}>
      <div>
        <h2 className="font-kanit text-xl font-semibold text-gray-900">Cash / Bank Snapshot</h2>
        <p className="text-sm text-gray-500">ยอดคงเหลือล่าสุดจาก cash-bank ledger</p>
      </div>

      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-950">
        <p className="font-medium">โหลดข้อมูล Cash / Bank ไม่สำเร็จชั่วคราว</p>
        <p className="mt-1 text-sm text-amber-900">
          ฐานข้อมูลตอบช้าหรือเชื่อมต่อไม่ทันเวลา ลองรีเฟรชหน้าอีกครั้ง หรือเปิดรายงานแยกเมื่อระบบกลับมาปกติ
        </p>
      </div>
    </section>
  );
...
async function getCashBankSnapshotData() {
  try {
    ...
    return { rows, totalBalance, cashBalance, bankBalance };
  } catch {
    return null;
  }
}
...
export default async function CashBankSnapshot({ compact = false }: Props) {
  const snapshot = await getCashBankSnapshotData();

  if (!snapshot) {
    return <SnapshotUnavailable compact={compact} />;
  }
    ...
    <section className={compact ? "space-y-3" : "space-y-4"}>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
          ...
          <p className="mt-1 font-kanit text-2xl font-bold text-slate-900">{fmt(snapshot.totalBalance)}</p>
        </div>
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
          <p className="text-xs text-emerald-700">เงินสด</p>
          <p className="mt-1 font-kanit text-2xl font-bold text-emerald-700">{fmt(snapshot.cashBalance)}</p>
        </div>
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
          <p className="text-xs text-blue-700">ธนาคาร</p>
          <p className="mt-1 font-kanit text-2xl font-bold text-blue-700">{fmt(snapshot.bankBalance)}</p>
        </div>
      </div>
