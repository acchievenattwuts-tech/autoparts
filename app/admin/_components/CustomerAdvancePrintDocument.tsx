...
import { PRINT_SECTION_BORDER_CLASS, formatPrintDate, formatPrintNumber, formatThaiBahtText, type PrintShopConfig } from "@/app/admin/_components/print/shared";

export default function CustomerAdvancePrintDocument({ advance, shopConfig, payments, copyLabel, rootClassName }: {
  advance: { advanceNo: string; advanceDate: Date | string; totalAmount: number; note?: string | null; status?: string; customer: { name: string; address?: string | null; phone?: string | null }; user?: { name?: string | null } | null };
  ...
  return <PrintDocumentRoot rootClassName={rootClassName ?? "mx-auto flex min-h-screen max-w-[900px] flex-col bg-white p-8 text-[13px] leading-snug"}>
    ...
    <div className="mt-auto"><PrintSignatureGrid columns={[{ label: "ผู้รับเงิน", dateText: "วันที่ ____/____/______", nameText: advance.user?.name ?? "", showNameLine: true }, { label: "ผู้จ่ายเงิน", dateText: "วันที่ ____/____/______", nameText: advance.customer.name, showNameLine: true }]} /></div>
  </PrintDocumentRoot>;
}
