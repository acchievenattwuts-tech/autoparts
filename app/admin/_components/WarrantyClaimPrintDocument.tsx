  ...
  return (
    <PrintDocumentRoot rootId={rootId} rootClassName={rootClassName}>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 text-xs">
        <div className={`space-y-0.5 rounded ${PRINT_SECTION_BORDER_CLASS} p-2`}>
          ...
          {claim.warranty.sale ? (
            <>
              <p>
                <span className="text-gray-700">เลขที่ใบขาย: </span>
                <span className="font-mono">{claim.warranty.sale.saleNo}</span>
              </p>
              <p>
                <span className="text-gray-700">วันที่ขาย: </span>
        ...
        <div className={`rounded ${PRINT_SECTION_BORDER_CLASS} p-2`}>
          ...
          <table className="w-full text-xs">
            <tbody>
              <tr>
                <td className="whitespace-nowrap py-0.5 pr-2 text-gray-700">เลขที่ใบเคลม</td>
              ...
              <tr>
                <td className="whitespace-nowrap py-0.5 pr-2 text-gray-700">สถานะ</td>
                <td>{statusLabel[claim.status] ?? claim.status}</td>
              </tr>
              <tr>
                <td className="whitespace-nowrap py-0.5 pr-2 text-gray-700">ประเภทเคลม</td>
      ...
      <table className="w-full border-collapse text-xs">
        ...
        <tbody>
          <tr>
            <td className={`${PRINT_TABLE_CELL_CLASS} text-center text-gray-700`}>1</td>
            <td className={`whitespace-nowrap ${PRINT_TABLE_CELL_CLASS} font-mono text-gray-700`}>
            ...
            <td className={PRINT_TABLE_CELL_CLASS}>
              ...
              <div className="text-[11px] text-gray-600">Lot No: {claim.warranty.lotNo ?? "-"}</div>
            </td>
      ...
      <div className="mb-4 grid grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] text-xs">
        <div className={`border-x border-b ${PRINT_BODY_BORDER_CLASS} p-2`}>
          <p className="mb-1 text-gray-700">อาการ / รายละเอียดการเคลม:</p>
          <p className="min-h-[4rem] text-gray-700">{claim.symptom ?? "-"}</p>
          {claim.note ? (
            <>
              <p className={`mt-3 pt-2 text-gray-700 ${PRINT_SECTION_TOP_BORDER_CLASS}`}>หมายเหตุ:</p>
        ...
        <div className={`border-r border-b ${PRINT_BODY_BORDER_CLASS} p-2`}>
          <div className="flex justify-between gap-3">
            <span className="text-gray-700">วันที่เริ่มประกัน</span>
            <span>{formatPrintDate(claim.warranty.startDate)}</span>
          </div>
          {claim.sentAt ? (
            <div className="flex justify-between gap-3 pt-1">
              <span className="text-gray-700">วันที่ส่งเคลม</span>
              <span>{formatPrintDate(claim.sentAt)}</span>
            </div>
          ) : null}
          {claim.resolvedAt ? (
            <div className="flex justify-between gap-3 pt-1">
              <span className="text-gray-700">วันที่ปิดเคลม</span>
              <span>{formatPrintDate(claim.resolvedAt)}</span>
            </div>
          ) : null}
          {claim.returnedAt ? (
            <div className="flex justify-between gap-3 pt-1">
              <span className="text-gray-700">วันที่ส่งคืนลูกค้า</span>
