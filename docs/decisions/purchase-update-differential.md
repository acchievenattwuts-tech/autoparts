...
- Batched per-product recalc (2026-06-14): หลัง batch create/update/reverse แล้ว คอขวดที่เหลือคือ loop ที่เรียก `recalculateStockCard()` ทีละ product และ `refreshProductPurchaseLastFields()` ที่ `product.update` ทีละ product — บิล 125 product = ~125×4 + 125 round-trip (recalc loop วัดได้ 17.7s local / ~53s prod). เพิ่ม `recalculateStockCardMany(tx, ids)` (lock/read/write StockCard/write Product อย่างละครั้ง, replay MAVG ต่อ product ด้วย `replayStockCardMavg` ที่ใช้ร่วมกับ single-product → ผลเหมือนเรียกทีละ product เป๊ะ) และเปลี่ยน `refreshProductPurchaseLastFields` เป็น bulk `UPDATE ... FROM (VALUES ...)` (COALESCE คง `purchaseUnitName` เดิมเมื่อไม่มี snapshot). วัดได้ recalc 125 product เหลือ 1.14s (15×). พิสูจน์ identical กับ per-row บนข้อมูลจริง 748 product ด้วย `prisma/scripts/verify-recalc-many.ts` ใช้ทั้ง `updatePurchase` และ purchase cancel path
