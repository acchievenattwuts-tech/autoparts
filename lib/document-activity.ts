...
function joinDescription(parts: Array<string | null | undefined>): string | undefined {
...
export function buildAuditActivityEvent(row: AuditActivityInput): DocumentActivityEvent | null {
...
export function buildRelationActivityEvent(input: RelationActivityInput): DocumentActivityEvent {
  return { ...input };
}

export function sortDocumentActivityEvents(events: DocumentActivityEvent[]): DocumentActivityEvent[] {
...
async function getReceiptRelationEvents(id: string): Promise<DocumentActivityEvent[]> {
  const db = await getDb();
...
async function getSupplierPaymentRelationEvents(id: string): Promise<DocumentActivityEvent[]> {
  const db = await getDb();
...
async function getSupplierAdvanceRelationEvents(id: string): Promise<DocumentActivityEvent[]> {
  const db = await getDb();
  ...
  return payments.map((item) => buildRelationActivityEvent({
