...
export type QuickSearchAccess = {
  ...
  customerAdvanceRefunds: boolean;
  supplierAdvances: boolean;
  supplierAdvanceRefunds: boolean;
...
type QuickSearchGroupKey =
  ...
  | "customer_advances" | "customer_advance_refunds"
  | "supplier_advances" | "supplier_advance_refunds"
  | "supplier_payments" | "expenses" | "warranty_claims"
...
export function buildQuickSearchGroups(rows: QuickSearchRow[],
): QuickSearchResultGroup[] {
  const configs: Array<{
    ...
    { key: "purchases", label: "ใบซื้อ", item: (r) => ({ id: `purchase:${r.id}`, label: r.label, sublabel: compactSublabel(r.secondary, r.extra, cancelledSuffix(r.status),
    ...
    { key: "customer_advance_refunds",
      label: "คืนเงินมัดจำลูกค้า",
      item: (r) => ({
        id: `customer-advance-refund:${r.id}`,
        label: r.label,
        sublabel: compactSublabel(r.secondary, r.extra, cancelledSuffix(r.status)),
        href: `/admin/customer-advance-refunds/${r.id}`,
      }),
    },
    {
    ...
    { key: "supplier_advance_refunds",
      label: "รับคืนเงินมัดจำซัพพลายเออร์",
      item: (r) => ({
        id: `supplier-advance-refund:${r.id}`,
        label: r.label,
        sublabel: compactSublabel(r.secondary, r.extra, cancelledSuffix(r.status)),
        href: `/admin/supplier-advance-refunds/${r.id}`,
      }),
    },
    {
    ...
    { key: "customers", label: "ลูกค้า", item: (r) => ({ id: `customer:${r.id}`, label: r.label, sublabel: compactSublabel(r.secondary, r.extra, r.isActive === false ? "(ปิดการใช้งาน)" : null,
        ), href: `/admin/customers/${r.id}`,
      }),
    },
    { key: "suppliers", label: "ซัพพลายเออร์", item: (r) => ({ id: `supplier:${r.id}`, label: r.label, sublabel: compactSublabel(r.secondary, r.isActive === false ? "(ปิดการใช้งาน)" : null,
...
export async function queryAdminQuickSearchRows(input: {
  ...
  add(input.access.customerAdvanceRefunds,
    Prisma.sql`(SELECT 57, 'customer_advance_refunds'::text, r.id, r."refundNo", c.name, a."advanceNo", r.status::text, NULL::boolean, r."refundDate", NULL::text FROM "CustomerAdvanceRefund" r JOIN "CustomerAdvance" a ON a.id=r."customerAdvanceId" JOIN "Customer" c ON c.id=a."customerId" WHERE r."refundNo" ILIKE ${pattern} ${input.docOnly ? Prisma.empty : Prisma.sql`OR a."advanceNo" ILIKE ${pattern} OR c.name ILIKE ${pattern}`} ORDER BY r."refundDate" DESC LIMIT ${input.take})`,
  );
  add(
    input.access.supplierAdvances, Prisma.sql`(SELECT 60, 'supplier_advances'::text, a.id, a."advanceNo", s.name, NULL::text, a.status::text, NULL::boolean, a."advanceDate", NULL::text FROM "SupplierAdvance" a LEFT JOIN "Supplier" s ON s.id=a."supplierId" WHERE a."advanceNo" ILIKE ${pattern} ${input.docOnly ? Prisma.empty : Prisma.sql`OR s.name ILIKE ${pattern}`} ORDER BY a."advanceDate" DESC LIMIT ${input.take})`,
  );
  add(input.access.supplierAdvanceRefunds,
    Prisma.sql`(SELECT 65, 'supplier_advance_refunds'::text, r.id, r."refundNo", s.name, a."advanceNo", r.status::text, NULL::boolean, r."refundDate", NULL::text FROM "SupplierAdvanceRefund" r JOIN "SupplierAdvance" a ON a.id=r."supplierAdvanceId" JOIN "Supplier" s ON s.id=a."supplierId" WHERE r."refundNo" ILIKE ${pattern} ${input.docOnly ? Prisma.empty : Prisma.sql`OR a."advanceNo" ILIKE ${pattern} OR s.name ILIKE ${pattern}`} ORDER BY r."refundDate" DESC LIMIT ${input.take})`,
  );
  add(
    input.access.supplierPayments, Prisma.sql`(SELECT 70, 'supplier_payments'::text, p.id, p."paymentNo", s.name, NULL::text, p.status::text, NULL::boolean, p."paymentDate", NULL::text FROM "SupplierPayment" p LEFT JOIN "Supplier" s ON s.id=p."supplierId" WHERE p."paymentNo" ILIKE ${pattern} ${input.docOnly ? Prisma.empty : Prisma.sql`OR s.name ILIKE ${pattern}`} ORDER BY p."paymentDate" DESC LIMIT ${input.take})`,
  );
