...
- **LINE: ลูกค้าซ่อนราคาถามราคา → ส่งเรื่องแอดมิน (2026-07-02)** — ต่อยอดจากข้อบน. ใน [lib/line-webhook-processor.ts](/D:/autoparts/lib/line-webhook-processor.ts) เมื่อ `showPrice===false` (ลูกค้าทั่วไป/unlinked) และลูกค้าถามราคา (`route.intent===PRICE_NEGOTIATION` หรือข้อความมี "ราคา/กี่บาท" — ตั้งใจไม่รวม PURCHASE_INTENT ล้วนอย่าง "เอาตัวนี้") จะส่งเรื่องให้แอดมินแจ้งราคาเอง 2 กรณี: (A) เทิร์นนี้ค้นเจอสินค้า → โชว์การ์ด ("สอบถามราคา") + note "ส่งเรื่องให้แอดมิน" แล้ว freeze `WAITING_ADMIN` + แจ้งแอดมิน (ปลด purchase-intent override เพื่อไม่ให้ข้ามการ์ด); (B) ถามราคาล้วนไม่มีสินค้า → forcedResponse handoff ตรง (`AI_PRICE_HIDDEN_HANDOFF`). อู่ (showPrice=true) เห็นราคาปกติ ไม่ handoff. reuse กลไก handoff เดิม (bell+Telegram notify). เพิ่มเทสต์ 3 เคส (processor รวม 56 ผ่าน). ตัวอย่างพฤติกรรมอ้างอิงจาก log จริงใน DB ("ราคาเท่าไร", "พอมีราคาให้มั้ยค่ะ", "หม้อน้ำ d-max ราคาเท่าไหร่")
