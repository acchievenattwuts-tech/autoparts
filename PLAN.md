...
- [~] **Phase 2 — Backfill public** (code เสร็จ + dry-run ผ่าน 2026-06-23): `prisma/scripts/backfill-public-images-to-blob.ts` (`npm run backfill:public-images-blob [-- --apply]`) — download จาก Supabase → `put` ขึ้น Blob path เดิม → rewrite URL ใน DB (Product.imageUrl, ProductImage.url, SiteContent shop_logo_url, User.signatureUrl, DeliveryProof.signature/photo, LineMessage.imageUrl); idempotent (ข้าม Blob URL แล้ว), ไม่ลบ Supabase, dump rollback JSON (gitignored), เขียน AuditLog UPDATE/ImageStorageMigration. **dry-run พบ 2,375 แถว (products 2,373 + line-chat 2)**
  ...
  - [ ] (บล็อก apply เหลือข้อเดียว) Supabase egress ต้องใช้ได้ — ตอนนี้ restrict ถึง 24 มิ.ย. (download 224MB = egress → 402). แผน: รอ reset 24 มิ.ย. → `npm run backfill:public-images-blob -- --apply` → dry-run ซ้ำ (เหลือ 0) → flip `IMAGE_STORAGE_PRODUCTS`+`IMAGE_STORAGE_LINE_CHAT=blob` ใน Vercel → เฝ้าดู egress ตก
- [~] **Phase 3 — Payment slips (private/PII)** (code เสร็จ + dry-run ผ่าน 2026-06-23, dormant หลัง flag `IMAGE_STORAGE_PAYMENT_SLIPS=supabase`):
