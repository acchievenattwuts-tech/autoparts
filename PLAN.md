...
- [x] **Admin report/export hardening follow-up (2026-05-18)** — fixed CSV/Excel export helpers so sales, purchases, credit notes, daily receipts, and daily payments export the full filtered dataset instead of silently reusing UI caps (`100`/`2000`). Kept report-page caps and existing subtotal/VAT/total formulas unchanged to avoid changing business meaning. Added `ReportTableShell` for the main report tables to improve mobile horizontal scrolling and dark-mode readability without altering data columns.
- [x] **Admin dashboard/cash-bank query pass (2026-05-18)** — changed Daily Operations top-products calculation from loading every sale item to Prisma `groupBy` plus product-name lookup, reduced visitor distinct lookups to selected distinct keys, and removed the hidden Cash/Bank ledger query path from normal dashboard render. No stock, AR/AP, or cash-bank movement logic changed.
- [x] **Password-change access fix (2026-05-18)** — `/admin/profile/change-password` now requires only an authenticated session so users forced to change password do not need unrelated `dashboard.view` permission before completing the password flow.
...
- [x] Run `npm run build` after each round; zero TS warnings.

- [x] **Admin sales/purchases totals query optimization (2026-05-18)** — changed `querySalesRowsTotals()` and `queryPurchaseRowsTotals()` to aggregate document subtotal/VAT and line-item totals directly in Prisma while reusing the same filter builder as the row queries. Preserved the previous footer meaning exactly: subtotal/VAT still come from document fields, total still comes from item totals, and `netAmount` is intentionally not substituted.
