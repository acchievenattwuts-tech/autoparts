...
- **LINE: ลูกค้าซ่อนราคาถามราคา → ส่งเรื่องแอดมิน (2026-07-02)** — ต่อยอดจากข้อบน. ใน [lib/line-webhook-processor.ts](/D:/autoparts/lib/line-webhook-processor.ts) เมื่อ `showPrice===false` (ลูกค้าทั่วไป/unlinked) และลูกค้าถามราคา (`route.intent===PRICE_NEGOTIATION` หรือข้อความมี "ราคา/กี่บาท" — ตั้งใจไม่รวม PURCHASE_INTENT ล้วนอย่าง "เอาตัวนี้") จะ **forcedResponse handoff ตรงไปหาแอดมิน** (`AI_PRICE_HIDDEN_HANDOFF`) freeze `WAITING_ADMIN` + แจ้งแอดมิน (bell+Telegram) โดย **ไม่โชว์การ์ด/รายการสินค้าซ้ำ** — การ์ดจะแสดงตอนลูกค้าค้นหาสินค้าปกติอยู่แล้ว การถามราคาคือการส่งต่อให้คน (ปรับหลังทดสอบจริง: เดิมโชว์การ์ดก่อน handoff แต่พบว่าซ้ำกับที่เคยแสดง จึงตัดออก). อู่ (showPrice=true) เห็นราคาปกติ ไม่ handoff. เพิ่มเทสต์ 3 เคส (processor รวม 56 ผ่าน). ตัวอย่างพฤติกรรมอ้างอิงจาก log จริงใน DB ("ราคาเท่าไร", "พอมีราคาให้มั้ยค่ะ", "หม้อน้ำ d-max ราคาเท่าไหร่")
