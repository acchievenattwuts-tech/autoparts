...
- **ส่วนลดระดับรายการ — SaleItem list price + line discount (2026-07-02)** — เพิ่ม 2 ฟิลด์ใน `SaleItem`: `unitListPrice` (ราคาตั้งก่อนลด/หน่วย) + `lineDiscount` (ส่วนลดรวมของบรรทัด, บาท) แบบ **additive ล้วน** — `salePrice` ยังเป็นราคาสุทธิต่อหน่วยและเป็น source of truth ของทุกการคำนวณ (กำไร/VAT/ยอด/ใบลดหนี้ไม่แตะ). Invariant: `unitListPrice ≥ salePrice`, `lineDiscount = (unitListPrice − salePrice) × qty` บังคับซ้ำใน Zod `.transform()` ฝั่ง server. UI หน้าขาย (`SaleForm.tsx`) แยก 3 คอลัมน์ ราคาตั้ง/ส่วนลดต่อหน่วย/ราคาสุทธิ คำนวณสองทางอัตโนมัติ + สรุป "ส่วนลดระดับรายการ" ท้ายบิล (แยกจาก "ส่วนลดท้ายบิล" = `Sale.discount`). Print (`SharedSalesDeliveryPrintDocument` + delivery/LIFF selects) โชว์ราคาตั้งขีดฆ่าเมื่อมีส่วนลด. รายงาน: `salesSummary.lineDiscountTotal` (memo อ้างอิง ไม่เข้าการคำนวณกำไรซ้ำ) + แถบใน `ReportsContent` + แถว CSV. Backfill `unitListPrice = salePrice` ให้ 99 row เดิมแล้ว (`prisma/scripts/backfill-saleitem-listprice.ts`). Audit snapshot เก็บ 2 ฟิลด์เพิ่ม. `db push` + `tsc --noEmit` ผ่าน
