...
const DateRangeFilter = ({ from, to }: DateRangeFilterProps) => {
  ...
  return (
    <div lang="en-GB" className={`flex items-center gap-2 text-sm transition-opacity ${isPending ? "opacity-50" : ""}`}>
      <span className="whitespace-nowrap text-gray-500">ช่วงวันที่</span>
      <input
        type="date"
        value={from}
        onChange={(e) => handleChange("from", e.target.value)}
        className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1e3a5f]/20"
        disabled={isPending}
      />
