    ...
    <footer className="bg-gray-900 text-white">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                ...
                <p className="text-[#f97316] text-xs">{shopSlogan}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 mt-5">
              ...
              {tiktokEnabled && tiktokUrl && (
                <a href={tiktokUrl} target="_blank" rel="noopener noreferrer"
                  className="w-9 h-9 rounded-lg bg-gray-900 flex items-center justify-center hover:opacity-80 transition-opacity"
                  aria-label="TikTok">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white">
              ...
              {/* Shopee */}
              {shopeeEnabled && shopeeUrl && (
                <a href={shopeeUrl} target="_blank" rel="noopener noreferrer"
                  className="w-9 h-9 rounded-lg bg-[#EE4D2D] flex items-center justify-center hover:opacity-80 transition-opacity"
                  aria-label="Shopee">
                  <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white">
              ...
              {/* Lazada */}
              {lazadaEnabled && lazadaUrl && (
                <a href={lazadaUrl} target="_blank" rel="noopener noreferrer"
                  className="w-9 h-9 rounded-lg bg-[#0F146D] flex items-center justify-center hover:opacity-80 transition-opacity"
                  aria-label="Lazada">
                  <svg viewBox="0 0 32 32" className="w-5 h-5 fill-white">
                    <path d="M16 2C8.268 2 2 8.268 2 16s6.268 14 14 14 14-6.268 14-14S23.732 2 16 2zm5.5 20h-11v-2h4.5V12h-4.5v-2h11v2h-4v8h4v2z" />
                  </svg>
                </a>
              )}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {["คอมเพรสเซอร์แอร์", "หม้อน้ำรถยนต์", "แผงคอนเดนเซอร์", "ท่อและสายแอร์", "อะไหล่อื่น ๆ"].map((item) => (
                <li key={item}>
                  <a href="#categories" className="text-gray-400 hover:text-white text-sm transition-colors">{item}</a>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-3">
              {shopPhone && (
                <li className="flex items-start gap-2.5">
                  ...
                  <a href={`tel:${shopPhone}`} className="text-gray-400 hover:text-white text-sm transition-colors">{shopPhone}</a>
                </li>
              )}
              <li className="flex items-start gap-2.5">
                <MapPin className="w-4 h-4 text-[#f97316] mt-0.5 flex-shrink-0" />
                <p className="text-gray-400 text-sm whitespace-pre-line">{shopAddress}</p>
              </li>
              <li className="flex items-start gap-2.5">
      ...
      <div className="border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-gray-500 text-sm">© {new Date().getFullYear()} {shopName} — สงวนลิขสิทธิ์</p>
          <p className="text-gray-600 text-xs">{shopSlogan}</p>
        </div>
      </div>
    </footer>
  );
