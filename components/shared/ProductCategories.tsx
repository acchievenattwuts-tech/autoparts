          ...
          <div className="flex min-w-max gap-3 sm:gap-4 lg:grid lg:min-w-0 lg:grid-cols-3 xl:grid-cols-4">
            {items.map((item, index) => (
              <ScrollReveal
                key={item.id}
                delay={index * 50}
                className="w-[220px] shrink-0 snap-start sm:w-[240px] lg:w-auto lg:shrink lg:snap-none"
              >
                <Link
                  href={item.href}
                  prefetch
        ...
        <div className="mt-5 rounded-2xl border border-[#06C755]/15 bg-white px-4 py-4 sm:px-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a
              ...
              className="sf-shine inline-flex items-center gap-2 self-start rounded-full bg-[#06C755] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#05a847]"
            >
