...
test("printing waits for streamed content and hydration, ignoring about:blank", async () => {
  ...
  const doc = { URL: "about:blank", nodeType: 9, querySelectorAll: (selector: string) => {
    ...
    return doc.URL === "about:blank" ? [] : [{ hasAttribute: () => false, getAttribute: (name: string) => name === "data-print-ready" && ready ? "true" : null }];
  } };
...
test("print readiness blocks a document whose pagination failed", async () => {
  const originalDocument = globalThis.document;
  Object.assign(globalThis, { document: { baseURI: "https://example.test" } });
  try {
    const root = { hasAttribute: (name: string) => name === "data-print-error", getAttribute: () => "จัดหน้าเอกสารไม่สำเร็จ" };
    const frame = { isConnected: true, contentDocument: { URL: "https://example.test/admin/sales/s1", querySelectorAll: () => [root] } } as unknown as HTMLIFrameElement;
    await assert.rejects(waitForPrintDocument(frame, "/admin/sales/s1"), /จัดหน้าเอกสารไม่สำเร็จ/);
