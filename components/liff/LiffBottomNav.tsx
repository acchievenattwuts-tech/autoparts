...
export default function LiffBottomNav({ active }: { active: string }) {
  return (
    <nav className="liff-bottom-nav border-t border-blue-100 bg-white/90 shadow-[0_-14px_34px_rgba(37,99,235,0.10)] backdrop-blur-xl dark:border-slate-700 dark:bg-slate-950/95 dark:shadow-[0_-14px_34px_rgba(0,0,0,0.30)]">
        ...
        {LIFF_BOTTOM_NAV_ITEMS.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              className={`flex min-h-[58px] flex-col items-center justify-center gap-1 px-1 text-[11px] font-semibold transition-colors ${
                isActive ? "text-blue-900 dark:text-sky-300" : "text-slate-500 dark:text-slate-400"
              }`}
            >
              <span
                className={`inline-flex h-8 min-w-12 items-center justify-center rounded-full transition-colors ${
                  isActive
